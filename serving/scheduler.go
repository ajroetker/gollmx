...
type batch struct {
	...
	positions           []int32 // per-element absolute positions (for RoPE), len == len(requests)
	cacheWritePositions []int32 // per-element cache write positions (for mask/KV write), nil if same as positions
}
...
func (s *scheduler) FormDecodeBatch(requests []*engineRequest) *batch {
	...
	for _, req := range requests {
		...
		b.positions = append(b.positions, int32(req.absPosition))
		if req.position != req.absPosition {
			// Compaction occurred: track cache write positions separately.
			if b.cacheWritePositions == nil {
				b.cacheWritePositions = make([]int32, 0, len(b.positions))
				// Backfill previous entries.
				for _, prev := range b.requests[:len(b.requests)-1] {
					b.cacheWritePositions = append(b.cacheWritePositions, int32(prev.position))
				}
			}
			b.cacheWritePositions = append(b.cacheWritePositions, int32(req.position))
		} else if b.cacheWritePositions != nil {
			b.cacheWritePositions = append(b.cacheWritePositions, int32(req.position))
		}
	}
	if len(b.requests) == 0 {
